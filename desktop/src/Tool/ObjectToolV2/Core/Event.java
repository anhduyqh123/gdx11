...
public class Event {
    ...
    public static DragActorListener dragInput;
    public static void InitDrag()
    {
        Scene.i.ui.addListener(new InputListener(){
            @Override
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (dragInput==null || button!=0) return false;
                dragInput.touchDown(event, x, y, pointer, button);
                return true;
            }
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer) {
                ...
                dragInput.touchDragged(event, x, y, pointer);
            }

            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                if (dragInput==null) return;
                dragInput.touchUp(event, x, y, pointer, button);
                dragInput = null;
            }
        });
    }
    public static class DragActorListener extends InputListener{
        private Vector2 p0 = new Vector2();
        private Vector2 p = new Vector2();
        private boolean debug;
        public IActor iActor;
        public DragActorListener(IActor iActor){
            this.iActor = iActor;
        }
        @Override
        public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
            debug = iActor.GetActor().getDebug();
            iActor.GetActor().setDebug(true);
            p0.set(x,y);
        ...
        @Override
        public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
            iActor.GetActor().setDebug(debug);
        }
    }
}
