...
uniform vec2 v2_center; // Mouse position
...
void main() 
{
	...
	float distance = distance(v_texCoords, v2_center);
	...
	if ( (distance <= (CurrentTime + shockParams.z)) && (distance >= (CurrentTime - shockParams.z)) ) {
    	...
    	vec2 diffUV = normalize(v_texCoords-v2_center);
